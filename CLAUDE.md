...
python3 -m pytest -q                                    # full suite (76 tests, ~0.3s)
python3 -m pytest tests/core/test_project.py -q         # one file
...
- `tests/core/test_acceptance_detection.py` runs the five real-world compose shapes in
  `tests/fixtures/compose/` through `detect_web` — add a fixture there when changing detection rules.
- `tests/guest/test_bootstrap_shell.py` shells `bash -n` over `bootstrap.sh` and asserts on its text
