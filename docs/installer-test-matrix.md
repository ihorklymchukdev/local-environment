...
| 7 | `dist\Omelet\omelet.exe selfcheck` run directly against the frozen build (any machine from cases 1-5) | That the bundled assets (`bootstrap.sh`, `stack.yml`, the nginx-hello template, `omelet.yaml`) actually resolve inside the frozen exe, not just in the source tree | Prints `OK` for every asset and exits 0 | UNRUN against this bundle | UNRUN against this bundle | **UNRUN.** The only recorded run (2026-08-19, Windows 11 26200, Python 3.14.0, PyInstaller 6.22.2) tested a different bundle — `bootstrap.sh`, `traefik.yml`, the nginx-hello template and `runtime.yaml`, resolved from `runtime/core/install.py` — and caught a real defect: the template resolved to `_internal\templates\` because `cli.py` is the frozen entry script and its `__file__` points at the bundle root. That fix still stands (`VERIFY_TEMPLATE` resolves from an imported module), but `traefik.yml` and `runtime.yaml` no longer exist and `stack.yml` has never been checked inside a frozen exe. Re-run before the next build ships. |
...
- **Case 7** is also run automatically inside `build.ps1` as part of the
  ...
  time on the build machine. The asset list changed after the last recorded
  run (`traefik.yml` and the host-side `runtime.yaml` are gone, `stack.yml`
  arrived), so the recorded PASS no longer describes anything that ships:
  the row stays UNRUN until someone runs it against the current bundle.
